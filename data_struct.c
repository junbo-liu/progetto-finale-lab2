...
rescuer_type_t* rescuer_search(char *type){
    for (int i = 0; i < rescuer_type_count; i++)
    {
        if (strcmp(all_rescuer_types[i]->rescuer_type_name, type) == 0)
        {
            return all_rescuer_types[i];
        }
    }
...
rescuer_digital_twin_t **all_rescuers = 0;
int rescuer_count = 0;
